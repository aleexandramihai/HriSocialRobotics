...
distortion_prompt = (
    "Today we are discussing the distortion called '{type}'. "
    "This distortion is defined as: {definition}. "
...
def main(session, details):
    ...
    for distortion in Distortions:
        filled_text = distortion_prompt.format(
            type=distortion["Type"],
            definition=distortion["Definition"],
            example=distortion["Example"]
        )
        distortion_response = generate_response(client, model, distortion_prompt, CONFIG)
        yield TTS_continuous(session, distortion_response)
        yield session.call("rie.dialogue.say", text="Start your answer with Yes or No")
        sentence = yield wait_response(session)
        processed_sentence = sentence.strip().lower()

        if processed_sentence.startswith(yes_words):
            print("Yes")
        elif processed_sentence.startswith(no_words):
            print("No")
        else:
            print("Unclear")
